...
function botes(){
  ...
  if(bote.length>0){
    if(bote[bote.length-1].bote.position.x<width/2){
    ...
    if(bote[i]) {
      bote[i].display()
    Matter.Body.setVelocity(bote[i].bote,{x:-5, y:0} )
    }
    }
  }
  else{
  ...
  function collision(indice){
    for(var i =0 ; i<bote.length; i ++){
      if(bote[i]!==undefined && bolas[indice!==undefined]){
         var colisao = Matter.SAT.collides(bote[i].bote, bolas[indice].bolla)
         if(colisao.collided){
          bote[i].remove(i)
         }
      }
